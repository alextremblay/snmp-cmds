from snmp_cmds import SNMPValueError, SNMPTimeout, SNMPUnknownHost
...
def test_basic(conf, oid):
    ...
    print snmp_command('snmpget', nortel1, oid['sysDescr'], '-v', '2c', '-c', cstring)
    print snmp_command('snmpget', nortel1, oid['ifNumber'], '-v', '2c', '-c', cstring)
    print snmp_command('snmpget', nortel1, oid['ifIndex'], '-v', '2c', '-c', cstring)
    print snmp_command('snmpget', cisco1, oid['sysDescr'], '-v', '2c', '-c', cstring)
    print snmp_command('snmpget', cisco1, oid['ifNumber'], '-v', '2c', '-c', cstring)
    print snmp_command('snmpget', cisco1, oid['ifIndex'], '-v', '2c', '-c', cstring)



def test_snmpget(conf, oid):
    comm_str = conf.get('auth', 'comm string')
    ip_list = [conf.get('test variables', 'cisco 1 ip'), conf.get('test variables', 'nortel 1 ip')]
    for ip in ip_list:
...
def test_api(conf, oid):
    ...
    print 'Session Test'
    print Session(hostname=ip, community=community_string, version=2).get(oid['sysName']).value